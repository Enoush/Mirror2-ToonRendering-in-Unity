Shader "Unlit/Rita/Socket"
{
    Properties
    {
        ...
        _SpecularScale("SpecularScale",Range(0.01,100)) = 1
        ...
        _RimScale("_RimScale",Range(0,50)) = 1
        ...
        [Header(Test)]
        _Testfa1("_Testfa1",Range(-20,20)) = 1
        _Testfa2("_Testfa2",Range(-20,20)) = 1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                NdotV = max(0,dot(normal_world,view_dir));
                float speScale = lerp(1,_SpecularScale,smoothness);//光滑度越高speScale越接近_SpecularScale
                half spe_fac = pow(saturate(dot(half_dir,normal_world)),speScale) * ao;
                ...
                * base_col * _SpecularIntensity * _SpecularColor.rgb;
                ...
                //最终漫反射
                half3 final_base_dif = toon_col.rgb * base_col.rgb * _LightColor0.rgb * _Testfa1;
                half3 final_sss_dif = _LightColor0.rgb * _DiffuseColor.rgb * _Testfa2;
                half3 final_dif = final_base_dif * (1-NdotV) + final_sss_dif * NdotV;

                #if DirDiffuse_ON
                final_dif = final_dif * base_col.rgb;
                #else
                final_dif = half3(0,0,0);
                #endif

                half3 final_col = final_dif + direct_spe * 0.5;
